...
from app.services.ranking import PostFeatures, exposure_metrics, rank_posts
from app.services.wellbeing import SessionFeatures, evaluate

app = FastAPI(
    title='NSosyal Bağlam API',
    version='0.4.1-context-guard',
    description='NSosyal integration prototype API — not an official NSosyal service. Python 3.14 target build.',
)
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
...
class AppealRequest(BaseModel):
    moderation_result_id: str = Field(min_length=1, max_length=64)
...
moderation_results: dict[str, dict] = {}
appeals: dict[str, dict] = {}
user_posts: list[PostFeatures] = []
nudge_responses: list[dict] = []


def _all_posts() -> list[PostFeatures]:
    return [*user_posts, *BASE_POSTS]


def _moderation_response(text: str) -> dict:
    result = moderation_engine.analyze(text).__dict__
    result_id = f'MOD-{len(moderation_results) + 1:04d}'
    payload = {'id': result_id, **result}
    moderation_results[result_id] = payload
    return payload
...
def health():
    return {
        ...
        'prototype': True,
        'app_version': '0.4.1-context-guard',
        'python_target': '3.14',
        'ui': 'single-origin-static',
        'moderation_engine': moderation_engine.version,
        'official_nsosyal_integration': False,
        'modules': {'baglam': True, 'adalet': True, 'denge': True},
    }


@app.get('/api/feed')
def feed():
    return rank_posts(_all_posts(), fair=True)


@app.post('/api/moderation/analyze')
def moderation_analyze(req: AnalyzeRequest):
    try:
        return _moderation_response(req.text)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))


@app.get('/api/moderation/results/{result_id}')
def moderation_result(result_id: str):
    if result_id not in moderation_results:
        raise HTTPException(status_code=404, detail='Moderasyon sonucu bulunamadı.')
    return moderation_results[result_id]


@app.post('/api/moderation/appeal')
def moderation_appeal(req: AppealRequest):
    if req.moderation_result_id not in moderation_results:
        raise HTTPException(status_code=404, detail='İtiraz edilecek moderasyon sonucu bulunamadı.')
    appeal_id = f'APL-{len(appeals) + 1:04d}'
    appeals[appeal_id] = {
        'id': appeal_id,
        'moderation_result_id': req.moderation_result_id,
        'reason': req.reason,
        'status': 'pending_review',
        'review_type': 'simulated_prototype_queue',
    }
...
def publish_post(req: PublishRequest):
    """Publish a user post inside the local prototype.

    A high-risk `review` decision is never silently bypassed. A `warn` decision can be
    consciously overridden, matching the visible "Yine de Yayınla" prototype action.
    """
    mod = _moderation_response(req.text)
    if mod['decision'] == 'review':
        return {
            'published': False,
            'status': 'queued_for_review',
            'moderation': mod,
            'message': 'Gönderi prototip inceleme akışına yönlendirildi.',
        }
    if mod['decision'] == 'warn' and not req.override_warning:
        return {
            'published': False,
            'status': 'warning_requires_confirmation',
            'moderation': mod,
            'message': 'BAĞLAM uyarısı nedeniyle yayın için kullanıcı onayı gerekiyor.',
        }

    post_id = 1000 + len(user_posts) + 1
    post = PostFeatures(
        post_id,
        'Deniz Aksoy',
        'small',
    ...
    return {
        'published': True,
        'status': 'published_local_prototype',
        'post_id': post_id,
        'moderation': mod,
        'message': 'Gönderi yerel prototip akışında yayınlandı.',
    }
...
def ranking(req: RankRequest):
    start = perf_counter()
    rows = rank_posts(_all_posts(), fair=req.fair)
    return {
        'method': 'fair' if req.fair else 'baseline',
        'items': rows,
        'metrics': exposure_metrics(rows),
        'latency_ms': round((perf_counter() - start) * 1000, 2),
        'ranking_version': 'fair-reranker-1.0' if req.fair else 'baseline-1.0',
    }
...
def ranking_compare():
    start = perf_counter()
    baseline = rank_posts(_all_posts(), fair=False)
    fair = rank_posts(_all_posts(), fair=True)
    return {
        'baseline': baseline,
        'fair': fair,
...
def ranking_explanation(post_id: int):
    rows = rank_posts(_all_posts(), fair=True)
    row = next((r for r in rows if r['id'] == post_id), None)
    if not row:
        raise HTTPException(status_code=404, detail='Gönderi bulunamadı.')
    reasons = []
    if row['relevance'] >= .85:
        reasons.append('İlgi alanlarıyla yüksek uyum')
    if row['quality'] >= .80:
        reasons.append('Kalite sinyali güçlü')
    if row['originality'] >= .90:
        reasons.append('Özgünlük sinyali güçlü')
    if row['under_exposure'] >= .5 and row['quality'] >= .7:
        reasons.append('Kaliteli fakat görece az görünür üretici')
    if not reasons:
        reasons.append('Temel ilgililik ve güncellik sinyalleri')
    return {
        'post_id': post_id,
        'reasons': reasons,
        'fairness_adjustment': row['fairness_adjustment'],
        'final_score': row['final_score'],
        'note': 'Takipçi sayısı tek başına artırma veya düşürme sinyali değildir.',
    }
...
def wellbeing_response(response: Literal['break_5', 'continue', 'hide_today']):
    entry = {
        'id': f'NR-{len(nudge_responses) + 1:04d}',
        'response': response,
        'stored': 'aggregate_demo_only',
    }
    nudge_responses.append(entry)
...
def metrics_ranking():
    # Keep report evidence stable: metrics use the fixed seven-post controlled dataset.
    baseline = rank_posts(BASE_POSTS, fair=False)
    fair = rank_posts(BASE_POSTS, fair=True)
    return {
        'status': 'ACTUAL_SYNTHETIC_PROTOTYPE',
        'baseline': exposure_metrics(baseline),
        'fair': exposure_metrics(fair),
    }
...
def metrics_wellbeing():
    ...
    return {
        'status': 'ACTUAL_CONTROLLED_SCENARIOS',
        'scenarios': {k: evaluate(v) for k, v in scenarios.items()},
    }


@app.post('/api/demo/reset')
def demo_reset():
    moderation_results.clear()
    appeals.clear()
    user_posts.clear()
    nudge_responses.clear()
